...
<h4 class="d-flex justify-content-center m-4">Для людини H та кожного експерименту, у якому вона брала участь, знайти скiльки разiв за вказаний перiод (з дати F по дату T) експеримент над нею проводили щонайменше N прибульцiв</h4>
  ...
  $alien = " ";
      ...
      $date2 = str_replace("/",".", $date2);
        ...
        if($alien != " ")
        {
          ...
          $sql = "SELECT count(experiment.*) as count_experiment_human_take_part, experiment.*
          from experiment
          ...
          and experiment.kidnapped_human_passport = '$alien'
          group by aliens_experiment.alien_isn, experiment.code
          having count(aliens_experiment.alien_isn) >= $n_num;
          ";
        }
