...
<h4 class="d-flex justify-content-center m-4">Вивести кораблi у порядку спадання сумарної кiлькостi еспериментiв, що були проведенi на кораблi за участi даного прибульця A протягом вказаного перiоду (з дати F по дату T)</h4>
  ...
  $alien = " ";
      ...
      $date2 = str_replace("/",".", $date2);
        ...
        if($alien != " ")
        {
          ...
          $sql = "SELECT spaceship.*, count(spaceship.*) as  count_experiments_held_on_spaceship
          from spaceship
          inner join experiment
            on experiment.spaceship_vin = spaceship.vin
          ...
          group by spaceship.vin 
          order by count_experiments_held_on_spaceship desc";
        }
