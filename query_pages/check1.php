...
<h4 class="d-flex justify-content-center">Для прибульця A знайти усiх людей, яких вiн викрадав хоча б N разiв за вказаний перiод
(з дати F по дату T)</h4>
<form class="d-flex justify-content-center" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
  <div class="form-row">
    ...
    <div class="col">
      <input type="text" class="form-control" name="n_num" placeholder="Enter N...">
    </div>
    <div class="col">
...
<table class="table table-bordered bg-secondary">
  ...
  $alien = " ";
  	if (!empty($_POST)){
  		...
  		$n_num = htmlspecialchars($_POST["n_num"]);
      ...
      $date2 = str_replace("/",".", $date2);
        ...
        if($alien != " ")
        {
         /*  $sql = "SELECT human_passport 
          FROM human_kidnapped_alien
          WHERE alien_isn = '$alien'
          AND (kidnapping_date > '$date' and kidnapping_date <'$date2')
          group by human_passport 
          having count(human_passport) >= $n_num;"; */

          $sql = "SELECT human.passport, human.isn, human.name,
          human.surname, human.birth_date from human
          INNER JOIN human_kidnapped_alien  
          ON human.passport = human_kidnapped_alien.human_passport
          where human_kidnapped_alien.alien_isn = '$alien'
          and (human_kidnapped_alien.kidnapping_date > '$date' 
             and human_kidnapped_alien.kidnapping_date < '$date2')
          group by human_kidnapped_alien.human_passport, human.passport
          having count(human_kidnapped_alien.human_passport) >= $n_num;";
        }
